...
def main_function(experiment_directory, continue_from,  iterations, marching_cubes_resolution, regularize):
    ...
    encoder = arch_encoder.ResNet(latent_size, specs["Depth"], norm_type = specs["NormType"], inputchannels=4).cuda()
    ...
    sdf_dataset_test = lib.data.RGBA2SDF_mask(
    ...
    for sdf_data, image, sil, azi, name in sdf_loader_test:
        ...
        image_export = 255*sil.permute(1,2,0).cpu().numpy()[...,0]
        imageio.imwrite(image_filename, image_export.astype(np.uint8))
        ...
        # T_cuda = camera_position_from_spherical_angles(distance=2.1, azimuth=azi.item(), elevation=10, device=device)
        # R_cuda = look_at_rotation(T_cuda,at=((0, 0, 0), ), up=((0, 1, 0),), device=device)
        R_cuda, T_cuda = look_at_view_transform(dist=2.1, elev=10, azim=(azi.item()-180))
        ...
        cameras = FoVPerspectiveCameras(device=device,znear=0.1, zfar=100.0, aspect_ratio=1.0, fov=35.0, R=R_cuda, T=T_cuda)
        ...
        mesh_filename = os.path.join(data_source, 'chairs/meshes', name[0] + ".obj")
        ...
        for e in range(iterations+1):
            ...
            # compute loss
            loss_silhouette = (torch.abs(silhouette - sil.cuda())).mean()
            ...
            with torch.no_grad():
                ...
                metric_chd_tmp, _ = chamfer_distance(meshes_gt_pts, meshes_dr_pts)
                metric_chd = torch.sqrt(metric_chd_tmp * 2) / 2
